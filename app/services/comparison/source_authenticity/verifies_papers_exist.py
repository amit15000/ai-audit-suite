"""Verifies papers exist score calculation for source authenticity detection."""
...
from app.services.comparison.source_authenticity.utils import JUDGE_SYSTEM_PROMPT, extract_json_bool
...
class VerifiesPapersExistScorer:
    ...
    async def calculate_score(
        ...
        if use_llm and len(paper_citations) > 0:
            try:
                prompt = f"""Check if the academic papers and research documents referenced in this response actually exist:
...
Return ONLY JSON: {{"verifies": <true/false>, "explanation": "<brief>"}}"""
                ...
                llm_result = extract_json_bool(judge_response, "verifies", result)
                # Use LLM result if it's more conservative (stricter)
                result = result and llm_result
            except Exception:
                pass
