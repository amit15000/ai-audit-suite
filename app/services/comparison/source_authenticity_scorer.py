    ...
    async def calculate_detects_fake_citations(
        ...
        if use_llm and citation_stats['total'] > 0:
            try:
                ...
                prompt = f"""Check if there are fake or fabricated citations in this response:
...
Return ONLY JSON: {{"detects": <true/false>, "explanation": "<brief>"}}"""
                ...
                if json_match:
                    ...
                    # Use LLM result (it's more accurate)
                    result = llm_result
            except Exception:
                pass
    ...
    async def calculate_confirms_legal_references(
        ...
        if use_llm and len(legal_references) > 0:
            try:
                prompt = f"""Check if the legal references (cases, statutes, regulations) in this response can be confirmed:
